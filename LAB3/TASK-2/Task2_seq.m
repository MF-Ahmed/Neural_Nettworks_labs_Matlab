...
[spec(i),sens(i),spec1(i),sens1(i),best_digit, worst_digit] = sequential(N(i),target_dataset,X)
fprintf('\n\rBest Digit = %d \n\r',best_digit);
fprintf('Worst Digit = %d \n\r',worst_digit);
    ...
    plot(sens);
    xlabel('Sensitivity Best Case - Sequential Case');
    subplot(2,2,2)
    plot(spec);
    xlabel('Specificity Best Case - Sequential Case');
    subplot(2,2,3)
    plot(sens1);
    xlabel('Sensitivity worst Case - Sequential Case');
    subplot(2,2,4)
    plot(spec1);
    xlabel('Specificity Worst Case - Sequential Case');
